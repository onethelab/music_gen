...
- 4~8개 스타일 태그가 적정
- **Style은 200자 이내로 작성** — 초과 시 뒷부분이 무시되므로 장르·템포를 앞에 배치
- BPM, 키(key)를 지정하면 정밀도 향상
- **"no X" 부정 프롬프트 사용 금지** — Style 필드는 네거티브 프롬프트가 아닌 태그 나열 방식이므로, `no trot`이라고 쓰면 오히려 "trot" 단어가 등장하여 해당 장르로 끌려갈 수 있음. 원치 않는 장르를 배제하려면 원하는 장르 키워드를 더 구체적으로 쌓아 밀도를 높인다
- **한국어 가사 곡의 Style에 "Korean" 단어 사용 금지** — Suno는 Lyrics의 언어를 자동 감지하여 해당 언어로 보컬을 생성함. Style에 "Korean female vocal"처럼 쓰면 한국 트로트/발라드 쪽으로 해석될 위험이 있음. 대신 보컬 질감으로 표현한다 (예: `Dark ethereal female vocal`, `Breathy female vocal`)
